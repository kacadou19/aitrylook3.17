    ...
    if (CanvasNodeSystem.mode === 'connect') {
        ...
        if (targetPort && (targetPort.classList.contains('can-connect-target') || targetPort.classList.contains('can-connect'))) {
            ...
            if (toId && fromId !== toId) {
                ...
                if (fromPort === 'left' || fromPort === 'left-ref') {
                    // 从左侧端口（输入端口）开始拖拽 - 反向连接
                    // toId 是源节点，fromId 是目标节点
                    if (fromNode && fromNode.type === NODE_TYPES.AI_LOCAL_TRANSFER) {
                        const slotType = fromPort === 'left-ref' ? 'reference' : 'source';
                        pushUndoState(captureCanvasState());
                        CanvasNodeSystem.connections.push({ from: toId, to: fromId });
                        addConnectionToLocalTransfer(toId, fromId, slotType);
                    } else {
                        addConnection(toId, fromId);
                    }
                } else if (targetPortType === 'left' || targetPortType === 'left-ref') {
                    // 拖到 LT 节点的左侧端口上
                    if (toNode && toNode.type === NODE_TYPES.AI_LOCAL_TRANSFER) {
                        const slotType = targetPortType === 'left-ref' ? 'reference' : 'source';
                        pushUndoState(captureCanvasState());
                        CanvasNodeSystem.connections.push({ from: fromId, to: toId });
                        addConnectionToLocalTransfer(fromId, toId, slotType);
                    } else {
    ...
    input.onchange = e => {
        Array.from(e.target.files).forEach((f, i) => {
            ...
            if (fromId) {
                ...
                setTimeout(() => {
                    if (fromNode && fromNode.type === NODE_TYPES.AI_LOCAL_TRANSFER) {
                        // 从局部迁移节点的左侧端口拉出去创建图片节点，图片连回 LT 节点
                        if (fromPort === 'left') {
                            addConnectionToLocalTransfer(newId, fromId, 'source');
                            CanvasNodeSystem.connections.push({ from: newId, to: fromId });
                            renderConnections();
                        } else if (fromPort === 'left-ref') {
                            addConnectionToLocalTransfer(newId, fromId, 'reference');
                            CanvasNodeSystem.connections.push({ from: newId, to: fromId });
                            renderConnections();
                        }
...
function renderLocalTransferNode(node) {
    ...
    el.innerHTML = `
        <div class="node-body rounded-2xl overflow-hidden shadow-lg" style="width:900px;background:#ffffff;border:1px solid #e5e7eb;position:relative;display:flex;flex-direction:column;">
            <!-- 标题栏 -->
            ...
            <!-- 主体：左侧原图大区域 + 右侧功能列 -->
            <div style="display:grid;grid-template-columns:420px 1fr;gap:8px;padding:8px 10px;min-height:460px;">
                <!-- 左：原图区域（高度自适应） -->
                <div style="position:relative;display:flex;flex-direction:column;gap:6px;">
                    <div id="lt-source-area-${node.id}" style="flex:1;min-height:380px;background:#f8fafc;border:1px solid #e5e7eb;border-radius:10px;overflow:hidden;position:relative;display:flex;align-items:center;justify-content:center;">
                        ${hasSource ? `<img src="${cropPreview}" style="width:100%;height:100%;object-fit:contain;"/>` : `<div style="text-align:center;color:#94a3b8;font-size:11px;"><div style="font-size:24px;opacity:0.4;margin-bottom:4px;">🖼️</div>连接图片到左侧端口</div>`}
                ...
                <div style="display:flex;flex-direction:column;gap:8px;">
                    ...
                    <div id="lt-func-area-${node.id}" style="display:flex;flex-direction:column;gap:6px;">
                        ...
                        <div data-status-tags style="display:flex;gap:6px;flex-wrap:wrap;align-items:center;">
                            ...
                            ${hasMask ? `<label onclick="event.stopPropagation()" style="display:flex;align-items:center;gap:3px;font-size:10px;color:#6b7280;cursor:pointer;user-select:none;"><input type="checkbox" id="lt-invert-mask-${node.id}" ${node.invertMask ? 'checked' : ''} onchange="event.stopPropagation();ltToggleInvertMask('${node.id}',this.checked)" style="width:12px;height:12px;cursor:pointer;"/>遮罩翻转</label>` : ''}
                        </div>
                        ...
                        <div style="display:flex;align-items:center;gap:6px;">
                            ...
                            <select id="lt-model-${node.id}" onchange="event.stopPropagation();ltUpdateModel('${node.id}',this.value)" onclick="event.stopPropagation()" onmousedown="event.stopPropagation()" style="flex:1;padding:4px 8px;font-size:11px;border:1px solid #e5e7eb;border-radius:6px;background:#f8fafc;color:#374151;outline:none;cursor:pointer;">
                                <option value="nano-banana-2" ${(node.ltModel||'nano-banana-2')==='nano-banana-2'?'selected':''}>nano-banana-2</option>
                                <option value="nano-banana-pro" ${node.ltModel==='nano-banana-pro'?'selected':''}>nano-banana-pro</option>
                            </select>
                            <span style="font-size:10px;color:#6b7280;white-space:nowrap;">清晰度</span>
                            <select id="lt-resolution-${node.id}" onchange="event.stopPropagation();ltUpdateResolution('${node.id}',this.value)" onclick="event.stopPropagation()" onmousedown="event.stopPropagation()" style="padding:4px 8px;font-size:11px;border:1px solid #e5e7eb;border-radius:6px;background:#f8fafc;color:#374151;outline:none;cursor:pointer;">
                                <option value="1024" ${(node.ltResolution||'4096')==='1024'?'selected':''}>1K</option>
                                <option value="2048" ${node.ltResolution==='2048'?'selected':''}>2K</option>
                                <option value="4096" ${(node.ltResolution||'4096')==='4096'?'selected':''}>4K</option>
                            </select>
                        </div>
                        <textarea id="lt-prompt-${node.id}" placeholder="描述局部迁移效果..." oninput="ltUpdatePrompt('${node.id}',this.value)" onclick="event.stopPropagation()" onmousedown="event.stopPropagation()" style="min-height:52px;border:1px solid #e5e7eb;border-radius:8px;padding:6px 10px;font-size:12px;resize:none;outline:none;box-sizing:border-box;background:#f8fafc;color:#374151;">${node.prompt || ''}</textarea>
                    </div>
                    <!-- 参考图 + 结果（左右各占50%） -->
                    <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;flex:1;">
                        <div id="lt-ref-area-${node.id}" style="aspect-ratio:1/1;background:#f8fafc;border:1px solid #e5e7eb;border-radius:10px;overflow:hidden;position:relative;">
                        ...
                        <div style="display:flex;flex-direction:column;">
                            <div id="lt-result-area-${node.id}" style="aspect-ratio:1/1;background:#f8fafc;border:1px solid #e5e7eb;border-radius:10px;overflow:hidden;display:flex;align-items:center;justify-content:center;">
...
function ltUpdateResolution(nodeId, val) {
    const node = CanvasNodeSystem.nodes.find(n => n.id === nodeId);
    if (node) node.ltResolution = val;
}

function ltToggleInvertMask(nodeId, checked) {
    const node = CanvasNodeSystem.nodes.find(n => n.id === nodeId);
    if (node) node.invertMask = !!checked;
}
...
function addConnectionToLocalTransfer(fromId, toId, slotType) {
    ...
    if (slotType === 'source') {
        // 原图槽位自动替换，清除旧的裁切/涂抹状态
        if (toNode.sourceImage && toNode.sourceImage.nodeId !== fromId) {
            // 移除旧连接
            CanvasNodeSystem.connections = CanvasNodeSystem.connections.filter(
                c => !(c.to === toId && c.from === toNode.sourceImage.nodeId)
            );
            toNode.cropRect = null;
            toNode.cropEnabled = false;
            toNode.cropPreviewUrl = null;
            toNode.maskDataUrl = null;
            if (toNode._ltUndoStack) toNode._ltUndoStack = [];
        }
...
function renderLTRefImages(nodeId) {
    ...
    node.referenceImages.forEach((ref, idx) => {
        ...
        if (!ref.transform) ref.transform = { x: 0, y: 0, scale: 1 };
        ...
        img.src = ref.url;
        // 用 object-fit:contain 让图片初始铺满区域，transform 叠加用户的拖拽/缩放
        img.style.cssText = `position:absolute;inset:0;width:100%;height:100%;object-fit:contain;transform-origin:center center;transform:translate(${t.x}px,${t.y}px) scale(${t.scale});pointer-events:none;user-select:none;`;
        img.draggable = false;
...
function bindLTRefDragEvents(nodeId, refIndex, wrap, imgEl) {
    ...
    const onMove = (e) => {
        ...
        imgEl.style.transform = `translate(${ref.transform.x}px,${ref.transform.y}px) scale(${ref.transform.scale})`;
    };
    const onUp = () => {
...
function _bindCropRectEvents(ltNodeId, cropRectEl, maxW, maxH) {
    ...
    const onMove = (e) => {
        ...
        if (mode === 'move') {
            ...
            let nw = startRect.width, nh = startRect.height;
            if (mode === 'se') {
                nw = Math.max(20, Math.min(maxW - startRect.x, startRect.width + dx));
                nh = Math.max(20, Math.min(maxH - startRect.y, startRect.height + dy));
            } else if (mode === 'sw') {
                nw = Math.max(20, startRect.width - dx);
                nh = Math.max(20, Math.min(maxH - startRect.y, startRect.height + dy));
                nx = Math.max(0, Math.min(startRect.x + startRect.width - 20, startRect.x + dx));
                nw = startRect.x + startRect.width - nx;
            } else if (mode === 'ne') {
                nw = Math.max(20, Math.min(maxW - startRect.x, startRect.width + dx));
                nh = Math.max(20, startRect.height - dy);
                ny = Math.max(0, Math.min(startRect.y + startRect.height - 20, startRect.y + dy));
                nh = startRect.y + startRect.height - ny;
            } else if (mode === 'nw') {
                nw = Math.max(20, startRect.width - dx);
                nh = Math.max(20, startRect.height - dy);
                nx = Math.max(0, Math.min(startRect.x + startRect.width - 20, startRect.x + dx));
            ...
            cropRectEl.style.left = nx + 'px';
            cropRectEl.style.top = ny + 'px';
            cropRectEl.style.width = nw + 'px';
            cropRectEl.style.height = nh + 'px';
        }
    };
    const onUp = () => {
        mode = null;
...
function _showBrushToolbar(nodeId) {
    ...
    tb.innerHTML = `
        ...
        <div style="display:flex;align-items:center;gap:4px;justify-content:flex-end;">
            ...
            <label onclick="event.stopPropagation()" style="display:flex;align-items:center;gap:3px;font-size:10px;color:#6b7280;cursor:pointer;user-select:none;margin-left:4px;"><input type="checkbox" id="lt-invert-mask-${nodeId}" ${node.invertMask ? 'checked' : ''} onchange="event.stopPropagation();ltToggleInvertMask('${nodeId}',this.checked)" style="width:12px;height:12px;cursor:pointer;"/>遮罩翻转</label>
        </div>
    `;
...
async function _ltGenerateCropPreview(ltNodeId) {
    ...
    try {
        ...
        const previewScale = Math.min(previewMaxSide / Math.max(sw, 1), previewMaxSide / Math.max(sh, 1), 1);
        const previewW = Math.max(1, Math.round(sw * previewScale));
        const previewH = Math.max(1, Math.round(sh * previewScale));
        cvs.width = previewW; cvs.height = previewH;
        const ctx = cvs.getContext('2d', { colorSpace: 'srgb' }) || cvs.getContext('2d');
        ctx.drawImage(srcImg, Math.max(0, sx), Math.max(0, sy), Math.min(sw, origW - Math.max(0, sx)), Math.min(sh, origH - Math.max(0, sy)), 0, 0, previewW, previewH);
...
async function applyLocalTransferNode(nodeId) {
    ...
    if (node.cropEnabled && node.cropRect) {
        ...
        const sw = Math.round((node.cropRect.width / displayW) * origW);
        const sh = Math.round((node.cropRect.height / displayH) * origH);
        cropPixel = {
            x: Math.max(0, sx),
            y: Math.max(0, sy),
            w: Math.min(sw, origW - Math.max(0, sx)),
            h: Math.min(sh, origH - Math.max(0, sy))
        };
    } else {
        cropPixel = { x: 0, y: 0, w: origW, h: origH };
    }

    // ── 扩图到正方形（白色填充，不拉伸，保持人物比例）──────────────────
    // 根据用户选择的清晰度决定输出尺寸（默认 4096）
    const API_SQUARE_SIZE = parseInt(node.ltResolution, 10) || 4096;
    const squareSide = Math.max(cropPixel.w, cropPixel.h);
    const padX = Math.round((squareSide - cropPixel.w) / 2);  // 裁切内容在正方形中的左偏移
    const padY = Math.round((squareSide - cropPixel.h) / 2);  // 裁切内容在正方形中的上偏移

    // 统一缩放到 4096x4096（无论原图多大，都放大/缩小到 4096）
    const scaleRatio = API_SQUARE_SIZE / squareSide;
    const apiSide = API_SQUARE_SIZE;
    const apiPadX = Math.round(padX * scaleRatio);
    const apiPadY = Math.round(padY * scaleRatio);
    const apiCropW = Math.round(cropPixel.w * scaleRatio);
    const apiCropH = Math.round(cropPixel.h * scaleRatio);

    const srcBlob = await fetchImageAsBlob(sourceUrl);
    const srcImg = await loadImageFromBlob(srcBlob);

    // 构建正方形 patch：白底 + 裁切内容居中（不拉伸）
    const patchCanvas = document.createElement('canvas');
    patchCanvas.width = apiSide; patchCanvas.height = apiSide;
    const patchCtx = patchCanvas.getContext('2d', { colorSpace: 'srgb' }) || patchCanvas.getContext('2d');
    patchCtx.fillStyle = '#ffffff';
    patchCtx.fillRect(0, 0, apiSide, apiSide);
    // 将裁切区域（原始像素坐标）按 scaleRatio 缩放后居中绘制
    patchCtx.drawImage(srcImg,
        cropPixel.x, cropPixel.y, cropPixel.w, cropPixel.h,
        apiPadX, apiPadY, apiCropW, apiCropH
    );

    // 处理 mask：同样居中放在正方形坐标系内
    let maskDataUrl = null;
    if (node.maskDataUrl) {
        const maskImg = await loadImageFromUrl(node.maskDataUrl);
        const maskCanvas = document.createElement('canvas');
        maskCanvas.width = apiSide; maskCanvas.height = apiSide;
        const maskCtx = maskCanvas.getContext('2d', { colorSpace: 'srgb' }) || maskCanvas.getContext('2d');
        // 白底（未涂抹区域为白色=不修改，涂抹区域为黑色=修改）
        maskCtx.fillStyle = '#000000';
        maskCtx.fillRect(0, 0, apiSide, apiSide);
        // mask 绘制在源节点（maskW × maskH）上，图片以 object-fit:contain 填充
        ...
        // cropPixel 在 mask 坐标系中的位置
        const scaleToMask = displayW / origW;
        const mCropX = cropPixel.x * scaleToMask + offsetX;
        const mCropY = cropPixel.y * scaleToMask + offsetY;
        const mCropW = cropPixel.w * scaleToMask;
        const mCropH = cropPixel.h * scaleToMask;
        // 将 mask 裁切区域缩放后居中绘制到正方形
        maskCtx.drawImage(maskImg, mCropX, mCropY, mCropW, mCropH,
            apiPadX, apiPadY, apiCropW, apiCropH);
        // 二值化
        const imageData = maskCtx.getImageData(0, 0, apiSide, apiSide);
        const shouldInvert = !!node.invertMask;
        for (let i = 0; i < imageData.data.length; i += 4) {
            let b = imageData.data[i + 3] > 6 ? 255 : 0;
            if (shouldInvert) b = b === 255 ? 0 : 255;
    ...
    if (node.referenceImages && node.referenceImages.length > 0) {
        refCompositeUrl = await buildLTRefComposite(node, apiSide, apiSide);
    }
    ...
    const resolution = `${apiSide}x${apiSide}`;  // 4096x4096
    console.log('[LT生成] origW:', origW, 'origH:', origH, 'cropPixel:', JSON.stringify(cropPixel));
    console.log('[LT生成] squareSide:', squareSide, 'API_SQUARE_SIZE:', API_SQUARE_SIZE, 'scaleRatio:', scaleRatio);
    console.log('[LT生成] padX:', padX, 'padY:', padY, 'apiSide:', apiSide);
    console.log('[LT生成] apiPadX:', apiPadX, 'apiPadY:', apiPadY, 'apiCropW:', apiCropW, 'apiCropH:', apiCropH);

    const selectedModel = node.ltModel || 'nano-banana-2';
    const runBtn = document.getElementById(`lt-run-btn-${nodeId}`);
    try {
        ...
        // 精确像素回切到原图（传入原始 mask 用于只回切涂抹区域）
        console.log('[LT回切] node.maskDataUrl 是否存在:', !!node.maskDataUrl);
        const maskInfo = node.maskDataUrl ? { rawMaskDataUrl: node.maskDataUrl, invertMask: !!node.invertMask } : null;
        const finalUrl = await buildLTCompositeExact(sourceUrl, imageUrl, cropPixel, origW, origH, { padX, padY, squareSide, scaleRatio }, maskInfo);
...
// 构建参考图合成（与原图 patch 完全相同尺寸的 4096x4096 正方形，参考图 contain 居中，白底）
async function buildLTRefComposite(node, targetW, targetH) {
    // targetW === targetH === 4096（与原图 patch 完全相同的正方形）
    const canvas = document.createElement('canvas');
    ...
    for (const ref of node.referenceImages) {
        try {
            ...
            // contain：等比缩放使图片完整放入 4096x4096 正方形画布，居中放置
            const imgRatio = img.naturalWidth / img.naturalHeight;
            let baseW, baseH;
            if (imgRatio > 1) {
                baseW = targetW; baseH = targetW / imgRatio;
            } else {
                baseH = targetH; baseW = targetH * imgRatio;
            }

            const drawW = baseW * scale;
            const drawH = baseH * scale;
            const drawX = (targetW - drawW) / 2 + t.x;
            const drawY = (targetH - drawH) / 2 + t.y;
...
// 精确像素回切：从正方形生成结果中取出有效区域贴回原图
async function buildLTCompositeExact(sourceUrl, patchUrl, cropPixel, origW, origH, squareInfo, maskInfo) {
    const srcBlob = await fetchImageAsBlob(sourceUrl);
    const srcImg = await loadImageFromBlob(srcBlob);
    ...
    if (squareInfo) {
        const { padX, padY, squareSide, scaleRatio } = squareInfo;
        const actualW = patchImg.naturalWidth;
        const actualH = patchImg.naturalHeight;
        console.log('[LT回切] squareSide:', squareSide, 'scaleRatio:', scaleRatio, 'padX:', padX, 'padY:', padY);
        console.log('[LT回切] cropPixel:', JSON.stringify(cropPixel));
        console.log('[LT回切] API返回图尺寸:', actualW, 'x', actualH);

        const apiSideSent = Math.round(squareSide * scaleRatio);
        const apiPadXSent = Math.round(padX * scaleRatio);
        const apiPadYSent = Math.round(padY * scaleRatio);
        const apiCropWSent = Math.round(cropPixel.w * scaleRatio);
        const apiCropHSent = Math.round(cropPixel.h * scaleRatio);

        const actualSide = Math.max(actualW, actualH);
        const returnScale = actualSide / apiSideSent;
        const realPadX = Math.round(apiPadXSent * returnScale);
        const realPadY = Math.round(apiPadYSent * returnScale);
        const realCropW = Math.round(apiCropWSent * returnScale);
        const realCropH = Math.round(apiCropHSent * returnScale);
        console.log('[LT回切] returnScale:', returnScale, 'realPadX:', realPadX, 'realPadY:', realPadY);

        // 先把生成图的有效区域绘制到临时 canvas（cropPixel 原图像素尺寸）
        const tmpCanvas = document.createElement('canvas');
        tmpCanvas.width = cropPixel.w; tmpCanvas.height = cropPixel.h;
        const tmpCtx = tmpCanvas.getContext('2d', { colorSpace: 'srgb' }) || tmpCanvas.getContext('2d');
        tmpCtx.drawImage(patchImg,
            realPadX, realPadY, realCropW, realCropH,
            0, 0, cropPixel.w, cropPixel.h
        );

        // 尝试用 mask 做精确回切（只替换涂抹区域）
        let maskApplied = false;
        if (maskInfo && maskInfo.rawMaskDataUrl) {
            try {
                console.log('[LT回切] 开始 mask 精确回切');
                // rawMaskDataUrl 是画笔在 lt-source-area 上画的原始 mask
                // 尺寸是 lt-source-area 的 DOM 像素尺寸，图片以 object-fit:contain 显示
                // 需要把它映射到原图像素坐标系的 cropPixel 区域
                const rawMaskImg = await loadImageFromUrl(maskInfo.rawMaskDataUrl);
                const rawMaskW = rawMaskImg.naturalWidth;
                const rawMaskH = rawMaskImg.naturalHeight;
                console.log('[LT回切] 原始mask尺寸:', rawMaskW, 'x', rawMaskH);

                // 原始 mask 是在 lt-source-area 上画的，图片以 object-fit:contain 填充
                // 需要计算图片在 area 中的实际显示区域
                const imgRatio = origW / origH;
                const areaRatio = rawMaskW / rawMaskH;
                let dispW, dispH, offX, offY;
                if (imgRatio > areaRatio) {
                    dispW = rawMaskW; dispH = rawMaskW / imgRatio;
                    offX = 0; offY = (rawMaskH - dispH) / 2;
                } else {
                    dispH = rawMaskH; dispW = rawMaskH * imgRatio;
                    offX = (rawMaskW - dispW) / 2; offY = 0;
                }

                // 构建原图像素坐标系下的 mask（origW x origH）
                const fullMaskCanvas = document.createElement('canvas');
                fullMaskCanvas.width = origW; fullMaskCanvas.height = origH;
                const fullMaskCtx = fullMaskCanvas.getContext('2d', { colorSpace: 'srgb' }) || fullMaskCanvas.getContext('2d');
                // 从原始 mask 的图片显示区域映射到原图全尺寸
                fullMaskCtx.drawImage(rawMaskImg,
                    offX, offY, dispW, dispH,
                    0, 0, origW, origH
                );

                // 从全尺寸 mask 中取出 cropPixel 区域
                const cropMaskCanvas = document.createElement('canvas');
                cropMaskCanvas.width = cropPixel.w; cropMaskCanvas.height = cropPixel.h;
                const cropMaskCtx = cropMaskCanvas.getContext('2d', { colorSpace: 'srgb' }) || cropMaskCanvas.getContext('2d');
                cropMaskCtx.drawImage(fullMaskCanvas,
                    cropPixel.x, cropPixel.y, cropPixel.w, cropPixel.h,
                    0, 0, cropPixel.w, cropPixel.h
                );

                // 读取 mask 数据，二值化
                const maskData = cropMaskCtx.getImageData(0, 0, cropPixel.w, cropPixel.h);
                const shouldInvert = !!maskInfo.invertMask;
                let whiteCount = 0, blackCount = 0;
                for (let i = 0; i < maskData.data.length; i += 4) {
                    // 画笔画的是黑色(R=0,G=0,B=0)带 alpha，未画的 alpha=0
                    // alpha > 6 表示涂抹过
                    let painted = maskData.data[i + 3] > 6 ? 255 : 0;
                    if (shouldInvert) painted = painted === 255 ? 0 : 255;
                    maskData.data[i] = painted;
                    maskData.data[i + 1] = painted;
                    maskData.data[i + 2] = painted;
                    maskData.data[i + 3] = 255;
                    if (painted > 128) whiteCount++; else blackCount++;
                }
                console.log('[LT回切] mask像素 - 涂抹:', whiteCount, '保留:', blackCount, '涂抹比例:', (whiteCount / (whiteCount + blackCount) * 100).toFixed(1) + '%', shouldInvert ? '(已翻转)' : '');

                // 用 mask 设置生成图的 alpha：涂抹区域不透明，未涂抹区域透明
                const genData = tmpCtx.getImageData(0, 0, cropPixel.w, cropPixel.h);
                for (let i = 0; i < genData.data.length; i += 4) {
                    genData.data[i + 3] = maskData.data[i]; // 白色=255=不透明=替换，黑色=0=透明=保留原图
                }
                tmpCtx.putImageData(genData, 0, 0);

                // 贴到原图上（只有涂抹区域会覆盖）
                ctx.drawImage(tmpCanvas, cropPixel.x, cropPixel.y);
                maskApplied = true;
                console.log('[LT回切] mask 精确回切完成 ✓');
            } catch (e) {
                console.warn('[LT回切] mask 回切失败，回退到整区域回切:', e);
                maskApplied = false;
            }
        }

        if (!maskApplied) {
            // 无 mask 或 mask 回切失败：整个裁切区域回切
            console.log('[LT回切] 整区域回切（无mask）');
        ...
        ctx.drawImage(patchImg, 0, 0, patchImg.width, patchImg.height,
            cropPixel.x, cropPixel.y, cropPixel.w, cropPixel.h);
    }
...
async function ltDownload(nodeId) {
    ...
    try {
        ...
        const filename = `local-transfer-${Date.now()}.png`;
        const saved = await downloadToDir(blob, filename);
        if (!saved) {
...
async function actionDownload(nodeId) {
    ...
    const filename = ensureFilenameExt(node.name || 'image.png', 'image/png');
    try {
        let blob;
        ...
        const saved = await downloadToDir(blob, ensureFilenameExt(filename, blob.type));
        if (!saved) {
            const objectUrl = URL.createObjectURL(blob);
            const a = document.createElement('a');
            a.href = objectUrl;
...
function downloadVideo(nodeId) {
    ...
    if (node.resultUrl.startsWith('data:') && typeof dataURLtoBlob === 'function') {
        doDownload(dataURLtoBlob(node.resultUrl));
        return;
    }
...
function updateConnectedInputImage(fromNodeId, imageUrl) {
    ...
    affectedConnections.forEach(conn => {
        ...
        // 局部迁移节点：更新原图或参考图
        if (toNode.type === NODE_TYPES.AI_LOCAL_TRANSFER) {
            const fromNode = CanvasNodeSystem.nodes.find(n => n.id === fromNodeId);
            const previewUrl = (fromNode && fromNode.previewUrl) || imageUrl;
            const origW = (fromNode && (fromNode.origW || fromNode.width)) || 512;
            const origH = (fromNode && (fromNode.origH || fromNode.height)) || 512;
            if (toNode.sourceImage && toNode.sourceImage.nodeId === fromNodeId) {
                // 更新原图槽位
                toNode.sourceImage.url = imageUrl;
                toNode.sourceImage.previewUrl = previewUrl;
                toNode.sourceImage.origW = origW;
                toNode.sourceImage.origH = origH;
                // 清除旧的裁切/涂抹状态（图片换了，之前的裁切区域不再有效）
                toNode.cropRect = null;
                toNode.cropEnabled = false;
                toNode.cropPreviewUrl = null;
                toNode.maskDataUrl = null;
                if (toNode._ltUndoStack) toNode._ltUndoStack = [];
                updateLTSourceDisplay(toNode.id);
            } else if (toNode.referenceImages) {
                // 更新参考图槽位
                const ref = toNode.referenceImages.find(r => r.nodeId === fromNodeId);
                if (ref) {
                    ref.url = imageUrl;
...
async function tryLookDownload(nodeId) {
    const node = CanvasNodeSystem.nodes.find(n => n.id === nodeId);
    if (!node || !node.resultUrl) return;
    const filename = `trylook_${nodeId}_${Date.now()}.png`;
    try {
        const resp = await fetch(node.resultUrl);
...
window.ltUpdateModel = ltUpdateModel;
window.ltUpdateResolution = ltUpdateResolution;
window.ltToggleInvertMask = ltToggleInvertMask;
...
// ==================== 下载保存目录配置 ====================
let _downloadDir = localStorage.getItem('aitrylook_download_dir') || '';
if (!window._downloadDirHandle) window._downloadDirHandle = null;
...
function togglePerformancePanel() {
    ...
    panel.innerHTML = `
        ...
        <div class="perf-section">
            <div class="perf-section-label">📥 下载保存目录</div>
            <div class="perf-path-row">
                <input type="text" class="perf-path-input" id="download-dir-input" placeholder="未设置，使用浏览器默认下载" value="${escapeHtmlAttr(_downloadDir)}" readonly />
                <button class="perf-browse-btn" onclick="browseDownloadDir()">选择</button>
            </div>
            <div class="perf-desc">设置后，下载生成图片时将自动保存到该文件夹。未设置则使用浏览器默认下载位置。</div>
        </div>
    ...
    try {
        ...
        if (_isTauri()) {
            try {
                ...
                if (e === '用户取消选择' || (e && e.message && e.message.includes('取消'))) return;
                console.warn('[DownloadDir] Tauri pick_directory 失败:', e);
            }
        }
        if (!dirPath && window.showDirectoryPicker) {
            try {
                const dirHandle = await window.showDirectoryPicker({ mode: 'readwrite' });
                window._downloadDirHandle = dirHandle;
                _downloadDir = dirHandle.name;
                localStorage.setItem('aitrylook_download_dir', _downloadDir);
                const input = document.getElementById('download-dir-input');
                if (input) input.value = _downloadDir;
                if (typeof showToast === 'function') showToast('已设置下载保存目录: ' + _downloadDir);
                return;
            } catch (e) {
                if (e.name === 'AbortError') return;
            }
        ...
        if (dirPath) {
            _downloadDir = dirPath;
            localStorage.setItem('aitrylook_download_dir', dirPath);
            window._downloadDirHandle = null;
            const input = document.getElementById('download-dir-input');
            if (input) input.value = dirPath;
            if (typeof showToast === 'function') showToast('已设置下载保存目录: ' + dirPath);
        }
    } catch (e) {
        if (e.name === 'AbortError') return;
        console.warn('[DownloadDir] 选择文件夹失败:', e);
    }
}

/**
 * 下载文件到自定义目录，如果未设置则走浏览器默认下载
 * @param {Blob} blob - 文件 blob
 * @param {string} filename - 文件名
 * @returns {Promise<boolean>} 是否使用了自定义目录保存
 */
async function downloadToDir(blob, filename) {
    const hasTauriPath = _isTauri() && _downloadDir && _downloadDir.length > 1;
    const hasBrowserHandle = !!window._downloadDirHandle;

    if (hasTauriPath) {
        try {
            const sep = _downloadDir.includes('\\') ? '\\' : '/';
            const filePath = _downloadDir + sep + filename;
            const arrayBuf = await blob.arrayBuffer();
            const uint8 = Array.from(new Uint8Array(arrayBuf));
            await _tauriInvoke('write_file_binary', { path: filePath, content: uint8 });
            if (typeof showToast === 'function') showToast('已保存到: ' + filePath);
            return true;
        } catch (e) {
            console.warn('[DownloadDir] Tauri 保存失败:', e);
        }
    }

    if (hasBrowserHandle) {
        try {
            const fileHandle = await window._downloadDirHandle.getFileHandle(filename, { create: true });
            const writable = await fileHandle.createWritable();
            await writable.write(blob);
            await writable.close();
            if (typeof showToast === 'function') showToast('已保存: ' + filename);
            return true;
        } catch (e) {
            console.warn('[DownloadDir] 浏览器保存失败:', e);
        }
    }
...
window.browseDownloadDir = browseDownloadDir;
window.downloadToDir = downloadToDir;
